...
import { BaseParser, type DebugLogger } from "../shared/base-parser";
import {
...
import type { PayloadRouteHandler } from "./payload-cms-types";
...
/**
 * Payload CMS Parser class
 */
export class PayloadCmsParser extends BaseParser {
	constructor(rootDir: string, options?: ParserOptions) {
		super(
			rootDir,
			{
				name: "Payload CMS",
				debugPrefix: "payload:parser",
				dependencies: [...PAYLOAD_DEPENDENCIES],
				filePatterns: [], // Payload uses config file discovery instead
	...
	protected async parseRoutes(): Promise<ParsedRoute[]> {
		...
		try {
			const srcPattern = path.join(this.rootDir, "src/**/*.{ts,tsx}");
			const rootPattern = path.join(this.rootDir, "*.{ts,tsx}");
			project.addSourceFilesAtPaths([srcPattern, rootPattern]);
			this.debug(`Added source files for import resolution`);
		} catch (error) {
			this.debug(`Failed to add source files: ${error}`);
		}

		// Find Payload config file
		const configFile = await this.findPayloadConfig(project);
		if (!configFile) {
			this.logger.warn("Payload CMS: No config file found");
			return [];
		}

		this.logger.info(`Payload CMS: Found config at ${configFile.getFilePath()}`);

		// Extract routes from config
		const handlers = this.parsePayloadConfig(configFile);
		this.logger.info(`Payload CMS: Extracted ${handlers.length} route handlers`);

		const routes = this.convertToRoutes(handlers);
	 ...
	 */
	private async findPayloadConfig(project: Project): Promise<SourceFile | null> {
		for (const pattern of PAYLOAD_CONFIG_PATTERNS) {
			const configPath = path.join(this.rootDir, pattern);
			try {
	 ...
	 */
	private parsePayloadConfig(sourceFile: SourceFile): PayloadRouteHandler[] {
		...
		const configObject = findPayloadConfigObject(sourceFile, this.debug);
		if (!configObject) {
			this.debug("Could not find Payload config object");
			return handlers;
		}

		// Extract API prefix (default is "/api")
		const apiPrefix = extractApiPrefix(configObject, this.debug);
		this.debug(`API prefix: ${apiPrefix}`);

		// Extract collections
		const collections = extractCollections(configObject, this.debug);
		this.logger.info(`Payload CMS: Found ${collections.length} collections`);
		for (const collection of collections) {
			const collectionRoutes = this.generateCollectionRoutes(
				collection,
				apiPrefix,
				filePath,
			);
		...
		// Extract globals
		const globals = extractGlobals(configObject, this.debug);
		for (const global of globals) {
			handlers.push(
				...this.generateGlobalRoutes(global, apiPrefix, filePath),
			);
		}

		// Extract custom endpoints
		const endpoints = extractEndpoints(configObject, this.debug);
		for (const endpoint of endpoints) {
			const handler = generateEndpointRoute(endpoint, apiPrefix, filePath, this.debug);
			if (handler) {
		...
		const defaultRoutes = generateDefaultEndpoints(apiPrefix, filePath, this.debug);
	 ...
	 */
	private generateCollectionRoutes(
		...
		const filePath = collection.sourceFile
			? path.relative(this.rootDir, collection.sourceFile)
			: configFilePath;
		...
		for (const op of COLLECTION_OPERATIONS) {
			...
			this.debug(`Generated collection route: ${op.method} ${routePath}`);
		}
		...
		if (collection.auth) {
			for (const op of AUTH_COLLECTION_OPERATIONS) {
				...
				this.debug(`Generated auth route: ${op.method} ${routePath}`);
			}
		}
		...
		if (collection.upload) {
			for (const op of UPLOAD_COLLECTION_OPERATIONS) {
				...
				this.debug(`Generated upload route: ${op.method} ${routePath}`);
			}
		}
	...
	private generateGlobalRoutes(
		...
		const filePath = global.sourceFile
			? path.relative(this.rootDir, global.sourceFile)
			: configFilePath;
		...
		for (const op of GLOBAL_OPERATIONS) {
			...
			this.debug(`Generated global route: ${op.method} ${routePath}`);
		}
		...
		for (const endpoint of global.endpoints) {
			...
			if (method) {
				...
				this.debug(`Generated custom global endpoint: ${method} ${routePath}`);
			}
		}
	 ...
	 */
	private convertToRoutes(handlers: PayloadRouteHandler[]): ParsedRoute[] {
...
export async function hasPayloadCMS(
	...
	const parser = new PayloadCmsParser(rootDir, options);
...
// =============================================================================
// Helper functions (module-level for reuse and reduced class complexity)
// =============================================================================
